...
func (s *applicationStateSuite) TestK8sServiceSharedHostname(c *tc.C) {
	...
	// Changing one owner's scope preserves the other owner's source facts.
	...
	c.Assert(s.upsertK8sService(c, "foo", "foo-provider", addresses), tc.ErrorIsNil)
...
func (s *applicationStateSuite) TestK8sServiceFailedReplacementIsAtomic(c *tc.C) {
	...
	err := s.upsertK8sService(c, "foo", "provider", network.ProviderAddresses{{
