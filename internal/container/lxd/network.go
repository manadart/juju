...
func (s *Server) verifyNICsWithAPI(nics map[string]device) error {
	...
	return errors.Errorf(
		"no network device found with nictype %q or %q, or network type %q"+
			"\n\tthe following devices were checked: %s"+
			"\nReconfigure lxd to use a network of type %q, %q, or %q.",
		nicTypeBridged, nicTypeMACVLAN, netTypeOVN,
		strings.Join(checked, ", "), nicTypeBridged, nicTypeMACVLAN, netTypeOVN)
}
