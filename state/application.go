...
func (a *Application) changeCharmOps(
	...
	checkStorageOps, upgradeStorageOps, storageConstraintsOps, err := a.newCharmStorageOps(cfg, ch, units, updatedStorageConstraints)
	if err != nil {
		return nil, errors.Trace(err)
	}
...
func (a *Application) newCharmStorageOps(
	...
	// invalid constraints.
	sb, err := NewStorageConfigBackend(a.st, cfg)
	if err != nil {
		return fail(err)
	}
	...
	upgradeStorageOps, err := a.upgradeStorageOps(cfg, ch.Meta(), oldMeta, units, newStorageConstraints)
	if err != nil {
		return fail(err)
	}
...
func (a *Application) AddUnit(
	...
	name, ops, err := a.addUnitOps(cfg, "", args, nil)
	if err != nil {
		return nil, err
	}
...
func (a *Application) UpsertCAASUnit(
	...
	err = a.st.db().Run(func(attempt int) ([]txn.Op, error) {
		...
		if unit == nil {
			return a.insertCAASUnitOps(cfg, args)
		}
...
func (a *Application) insertCAASUnitOps(
	...
	_, addOps, err := a.addUnitOps(cfg, "", args.AddUnitParams, nil)
	if err != nil {
		return nil, errors.Trace(err)
	}
...
func (op *AddUnitOperation) Build(attempt int) ([]txn.Op, error) {
	...
	name, addOps, err := op.application.addUnitOps(op.cfg, "", addUnitArgs, nil)
	if err != nil {
		return nil, errors.Trace(err)
	}
